...
def summarize_stats(clust_file, data_path, out_name, max_clust):
    ...
    for line in f:
        l = line.rstrip().split(',')
        kids.append(int(l[0]))
        kdigos.append(int(np.max(np.array(l[1:], dtype=int))))
    ...
    clusters = np.repeat(50, len(all_ids))
    for i in range(len(cids)):
        idx = cids[i]
        loc = np.where(all_ids == idx)[0][0]
        clusters[loc] = clust[i]
    ...
    f.write('Patient_ID,Cluster_ID,Max_KDIGO,Died_Inp\n')
