...
# --------------------------------------------------- PARAMETERS ----------------------------------------------------- #
data_file = '../RESULTS/icu/7days_070118/features.h5'
# features = ['sofa_norm', 'apache_norm', 'all_clinical', 'all_trajectory_clusters', 'everything_clusters']
# features = ['all_trajectory_clusters', 'everything_clusters']
features = ['sofa_norm', 'apache_norm', 'all_clinical']
lbl_list = ['died_inp', ]
basepath = '../RESULTS/icu/7days_070118/classification/'

# dm_tags = ['nna1', 'nna2', 'cca1', 'cca2', 'cca4', 'cna1', 'cna2', 'cna4', 'nca1', 'nca2']
dm_tags = ['nna1',]
# dm_tags = [dm_tags[x] + '-2' for x in range(len(dm_tags))]
...
tpath = 'all'
mort_exc = (0, 0)       # tuple indicating when patients should be removed based on their mortality date
    ...
    for lbls in lbl_list:
        ...
        for model in models:
            ...
            if model == 'svm':
                ...
                tuned_parameters = [{'n_estimators': [5, 500],
                                     'criterion': ['gini', ],
                                     'max_features': ['sqrt', ]}]
            ...
            for score in scores:
                ...
                for feature in features:
                    if type(feature) == list:
                        X = np.hstack([fg[x] for x in feature])
                        tstr = feature[0]
                        for i in range(1, len(feature)):
                            tstr += ', ' + feature[i]
                    else:
                        # X = fg[feature + '_' + dm_tag][:]
                        # tstr = feature + '_' + dm_tag
                        X = fg[feature][:]
                        tstr = feature
                    try:
                        X = X[pt_sel, :]
                    except:
                        X = X[pt_sel].reshape(-1, 1)
                    print('Evaluating classification performance using: %s' % tstr)
                    # Load features and labels

                    if not os.path.exists(tpath + tstr + '/'):
                        os.mkdir(tpath + tstr + '/')
                    else:
                        if os.path.exists(tpath + tstr + '/evaluation_summary.png'):
                            continue
                    ...
                    if n_pos < n_neg:
                        ...
                        pos_test = np.random.permutation(np.setdiff1d(pos_idx, pos_train))[:len(neg_test)]
                    ...
                    log_file = open(tpath + tstr + '_log.txt', 'w')
                    log = True
                    ...
                    for i, (train_idx, val_idx) in enumerate(skf.split(vX, vy)):
                        ...
                        if not os.path.exists(basepath + tstr):
                            os.mkdir(basepath + tstr)
                        ...
                        plt.tight_layout()
                        plt.savefig(tpath + tstr + '/fold' + str(i + 1) + '_evaluation.png')
                        plt.close(fig)
                    ...
                    plt.savefig(tpath + tstr + '/evaluation_summary.png')
            basepath = temp
        basepath = temp1
    basepath = opath
