...
#            'absmismatch_normBC_popcoord_shift1', 'custmismatch_extension_a1E+00_normBC',
...
hlims = [15,]                                           # Height limit for cutting dendrogram
...
pt_sel = np.array((), dtype=int)
for x in kdigo_sel:
    pt_sel = np.union1d(pt_sel, np.where(max_kdigo == x)[0])
...
for dm_tag in dm_tags:
    ...
    # if not os.path.exists(save_path):
    #     os.mkdir(save_path)
    if dm_tag == 'descriptive_features':
        feats = load_csv(os.path.join(resPath, 'features', 'individual', 'descriptive_features.csv'), ids, skip_header=True)
        dm = pdist(feats)
        sqdm = squareform(dm)
    else:
    ...
    if len(kdigo_sel) < 4:
        tpath = os.path.join(save_path, 'kdigo_%s' % ''.join('%d' % x for x in kdigo_sel))
        if not os.path.exists(tpath):
            os.mkdir(tpath)
    else:
        tpath = save_path
    eps = cluster_trajectories(f, ids, max_kdigo, sqdm, meta_grp=meta_grp, eps=eps, p_thresh_l=p_threshs,
                                ...
                                skip_db=skip_db, kdigos=kdigos, days=days)
