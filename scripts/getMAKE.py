...
for npts in [1, 2]:
    for buf in [0, 30]:
        for ref in ['admit', 'disch']:
            ...
            esrd_scm = np.max(m90[:, 3:5], axis=1)
            esrd_usrds = m90[:, 5]
            ...
            dropIdx = np.setdiff1d(aliveIdx, np.where(esrd_scm)[0])
            dropIdx = np.setdiff1d(dropIdx, np.where(ddep)[0])
            dropIdx = np.setdiff1d(dropIdx, np.where(esrd_usrds)[0])
            ...
            print('%s,%d,%d,%d (%.2f),%d (%.2f),%d (%.2f),%d (%.2f),%d (%.2f),%d (%.2f),%d (%.2f),%d (%.2f),%d (%.2f),%d (%.2f)' %
                  (ref, buf, npts, len(diedIdx), float(len(diedIdx)) / len(ids) * 100, np.sum(esrd_scm),
                   np.sum(esrd_scm) / (len(aliveIdx)) * 100, np.sum(esrd_usrds),
                   np.sum(esrd_usrds) / (len(aliveIdx)) * 100, np.sum(ddep), np.sum(ddep) / (len(aliveIdx)) * 100,
